...
class NSEquation_Classic(baseclass.BasePDE):
    ...
    def __init__(self, nu=1, bbox=[0, 8, 0, 8], circles=[[6, 6, 1.0], [2, 1.0, 0.5]], linear=False):
        ...
        # traing config
        self.training_points(domain=4096, boundary=512)
...
class NSEquation_Long(baseclass.BaseTimePDE):
    ...
    def __init__(self, nu=1 / 100, bbox=[0, 2, 0, 1, 0, 5]):
        ...
        def initial_f(x):
            return torch.sin(np.pi * x[:, 0]) * torch.sin(np.pi * x[:, 1]) * torch.sin(np.pi * x[:, 2])
        ...
        def boundary_in(x, on_boundary):
            return on_boundary and np.isclose(x[0], bbox[0]) and 0 <= x[1] <= 0.5
        ...
        # training config
        self.training_points(domain=4096, boundary=512, initial=1024)
...
class NSEquation_LidDriven(baseclass.BasePDE):
    ...
    def __init__(self, nu=1 / 100, bbox=[0, 1, 0, 1]):
        ...
        self.add_bcs([{
            'component': 0,
            'function': (lambda x: x[:, 0:1]*(5 - x[:, 0:1])),
            'bc': boundary_top,
            'type': 'dirichlet'
        }, {
            'component': 1,
            ...
            'values': np.array([[0]]),
            'type': 'pointset'
        }])  # initial value of pressure (Zhang Yichi) 
...
class NSEquation_FourCircles(baseclass.BasePDE):
    ...
    def __init__(
        ...
        self.load_ref_data("ref/ns_4_obstacle.dat")
