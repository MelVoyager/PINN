...
class GrayScottEquation(baseclass.BaseTimePDE):
    ...
    def __init__(self, bbox=[-1, 1, -1, 1, 0, 10], b=0.04, d=0.1, epsilon=(2e-5, 1e-5)):
        super().__init__()
        # output dim
        self.output_dim = 2
        ...
        def ic_func(x, component):
            if component == 0:
                return 1 - np.exp(-20 * ((x[:, 0] + 0.05)**2 + (x[:, 1] + 0.02)**2))
            else:
                return np.exp(-20 * ((x[:, 0] - 0.05)**2 + (x[:, 1] - 0.02)**2))
...
class KuramotoSivashinskyEquation(baseclass.BaseTimePDE):
    ...
    def __init__(self, bbox=[0, 2 * np.pi, 0, 5], alpha=100 / 16, beta=100 / (16 * 16), gamma=100 / (16**4)):
        super().__init__()
        # output dim
        self.output_dim = 1
