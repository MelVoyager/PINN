...
def plot_heatmap(x, y, z, path=None, vmin=None, vmax=None, title='', xlabel='x', ylabel='y', show=False):
    '''
    ...
    fig = plt.imshow(vals, extent=[np.min(x), np.max(x), np.min(y), np.max(y)], aspect='equal', interpolation='bicubic', vmin=vmin, vmax=vmax)
...
def plot_3dheatmap(x, y, z, values, path, split=6, title='', xlabel='x', ylabel='y', zlabel='z'):

    bbox = [x.min(), x.max(), y.min(), y.max(), z.min(), z.max()]
    xx, yy, zz = np.linspace(bbox[0], bbox[1]), np.linspace(bbox[2], bbox[3]), np.linspace(bbox[4], bbox[5], split)
    xx, yy, zz = np.meshgrid(xx, yy, zz)
    vals = interpolate.griddata(np.array([x, y, z]).T, values, (xx, yy, zz))
...
def plot_state(pde, train_state, output_dir, is_best=False, lossy=False):
    ...
    # 2D
        ...
        for i in range(y_dim):
            ...
            plot_heatmap(x_test[:, 0], x_test[:, 1], y_test[:, i], save_path, title=f"Prediction for {pde.output_config[i]['name']}")
            if x_data is not None:
                if pde.ref_sol is not None:
                    plot_heatmap(
                        x_test[:, 0], x_test[:, 1], y_test[:, i] - y_data[:, i], error_save_path, title=f"Error for {pde.output_config[i]['name']}"
                    )
                else:
                    y = interpolate.griddata(x_test, y_test[:, i], x_data, method='cubic')
                    y0 = interpolate.griddata(x_test, y_test[:, i], x_data, method='nearest')
                    y[np.isnan(y)] = y0[np.isnan(y)]
                    plot_heatmap(x_data[:, 0], x_data[:, 1], y - y_data[:, i], error_save_path, title=f"Error for {pde.output_config[i]['name']}")
    # 3D
    elif x_dim == 3:
        for i in range(y_dim):
            save_path = output_dir + pde.output_config[i]['name'] + '_pred.png'
            ...
            if x_data is not None:
                if pde.ref_sol is not None:
                    ...
                    else: # using training points (if data is **densy**)
