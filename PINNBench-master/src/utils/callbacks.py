...
class PlotCallback(Callback):
    ...
    def __init__(self, log_every=None, verbose=False, lossy=False):
    ...
    def log(self, save_path):
        ...
        plot.plot_state(self.model.pde, train_state, save_path, is_best=False, lossy=self.lossy)
    ...
    def on_epoch_end(self):
        ...
        if self.log_every is None or self.valid_epoch % self.log_every != 0:
            return
        if self.verbose:
            logger.info("Plotting at epoch {} ...".format(self.valid_epoch))
    ...
    def on_train_end(self):
        if self.verbose:
            logger.info("Plotting at train end ...")
        self.log(self.base_save_path)
...
class LossCallback(Callback):
    ...
    def on_epoch_end(self):
        ...
        if self.verbose:
            ...
            print('Unweighted Loss: {}  {} Weights: {}' \
                  .format(list_to_str(loss_train), list_to_str(loss_test), list_to_str(loss_weight)))
    ...
    def on_train_end(self):
        ...
        if self.weighted:
            ...
            np.savetxt(save_path + "loss.txt", loss, header=f"step, loss_train, loss_test")
            ...
            np.savetxt(save_path + "loss.txt", loss, header=f"step, loss_train, loss_test, loss_weight")
            plot.plot_loss_history(self.model.pde, loss_history, save_path)
    ...
    def on_train_begin(self):
        ...
        if pde.ref_sol is not None:
            if pde.input_dim == 2:
                ...
                sample_points = int(1e5**(1 / pde.input_dim))

            xlist = [np.linspace(pde.bbox[i * 2], pde.bbox[i * 2 + 1], sample_points) for i in range(pde.input_dim)]
            self.test_x = np.stack(map((lambda x: x.reshape(-1)), np.meshgrid(*xlist))).T
