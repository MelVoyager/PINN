...
from GaussJacobiQuadRule_V3 import GaussLobattoJacobiWeights_torch
os.chdir(sys.path[0])
...
def integrate_u(Q, u):
    X, W = GaussLobattoJacobiWeights_torch(Q, 0, 0)
    X = X.view(-1, 1)
    Y = X.view(1, -1)
    W_x = W.view(-1, 1)
    W_y = W.view(1, -1)
    ...
    u_values = u(X, Y)
    integral = torch.sum(W_x * W_y * u_values)
...
def wrapper1(x, y):
    return gradient_u(x, y) * new_lengendre.v(1, 2, x, y)

def wrapper2(x, y):
    return f(x, y) * new_lengendre.v(0, 2, x, y)

# s1 = torch.sum(gradient_u(xx, yy) * new_lengendre.v(1, 2, xx, yy))
s1 = integrate_u(10, wrapper1)
s2 = integrate_u(10, wrapper2)
