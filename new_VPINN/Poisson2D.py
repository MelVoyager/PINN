...
def LaplaceWrapper(x, y, index=None, k1=1, k2=1):
    ...
    result = torch.bmm((d2x + d2y).view(100, grid_num ** 2, 1), (test_func.test_func(0, x, y)).view(100, 1, 1))
    return result
...
def DeltaWrapper(x, y, index, k1=1, k2=1):
    ...
    return result.view(n, 100, 1)

def loss_interior_2(net, index=None, k1=1, k2=1):
    
    int1 = torch.abs(quad_integral.integral(DeltaWrapper, k1, k2, index))
...
def train():
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5e3, gamma=0.8)
    ...
    for i in tqdm(range(100)):
        ...
        optimizer.zero_grad()
        # loss_tot += loss_interior_1(net) + coef * (loss_bc1(net) + loss_bc2(net) + loss_bc3(net) + loss_bc4(net))
