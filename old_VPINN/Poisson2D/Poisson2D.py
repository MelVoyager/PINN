...
def grid(n=N):
    ...
    y = torch.linspace(-1, 1, n).to(device)
...
def gradient_v(x, y, k1=1, k2=1):
    return torch.cat([lengendre.v_prime(x, k1) * lengendre.v(y, k2), lengendre.v(x, k1) * lengendre.v_prime(y, k2)], dim=1)
...
def loss_interior_2(net, k1=1, k2=1):
    ...
    int1 = torch.abs(torch.sum(net_gradient_u * gradient_v(x, y, k1, k2))) * ((2 / N) ** 2)
    int2 = torch.abs(torch.sum(condition * v(x, y, k1, k2))) * ((2 / N) ** 2)
    int3 = torch.abs(torch.sum(gradient_u(x, y) * gradient_v(x, y, k1, k2))) * ((2 / N) ** 2)
    # print(int1, int2, int3)
    loss2[k1-1].append(loss(int1, int2).item())
    return loss(int1, int2)
...
for i in tqdm(range(10000)):
    ...
    loss_tot = loss_interior_1(net, 1) + loss_interior_1(net, 2) \
                + loss_interior_2(net, 1, 1) + loss_interior_2(net, 1, 2) + loss_interior_2(net, 1, 3)\
                + loss_interior_2(net, 2, 1) + loss_interior_2(net, 2, 2) + loss_interior_2(net, 2, 3)\
                + loss_interior_2(net, 3, 1) + loss_interior_2(net, 3, 2) + loss_interior_2(net, 3, 3)\
