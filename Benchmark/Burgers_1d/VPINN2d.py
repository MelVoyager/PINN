...
class VPINN:
    def __init__(self, layer_sizes, f, bc, type=0, Q=10, grid_num=6, test_fcn_num=5, device='cpu', load=None):
        ...
        if load:
            ...
            # self.net.load_state_dict(self.net)
        else:
            self.net = MLP(layer_sizes).to(device)
        xs = []
        ys = []
    ...
    def lhsWrapper(self, x, y):
        u = self.net(torch.cat([self.grid_xs, self.grid_ys], dim=1))
        u_y = self.gradients(u, self.grid_ys, 1)
        u_x = self.gradients(u, self.grid_xs, 1)
        u_xx = self.gradients(u, self.grid_xs, 2)
        lhs = u_y + u * u_x - 0.01 / torch.pi * u_xx
        result = torch.einsum('mc,nc->mnc', \
